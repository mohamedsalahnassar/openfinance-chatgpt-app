...
function buildWidgetHtml(variant, apiBaseOverride) {
...
const paymentWidget = {
    ...
    description: "User-friendly payment flow that guides through amount entry, consent creation, bank authentication, and payment initiation with automatic data pre-filling.",
...
const tools = [
    ...
    registerTool({
        spec: {
            ...
            description: "Opens an intuitive payment flow for single instant payments through the AlTareq Model Bank. User enters payment amount, authorizes through the bank, and completes the payment with automatic data pre-filling between steps.",
            inputSchema: {
                type: "object",
                properties: {},
                additionalProperties: false,
...
function serveInlinedWidget(res, req, variant) {
    res.setHeader("Access-Control-Allow-Origin", "*");
